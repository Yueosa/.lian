...
我的环境里有大量自己编写的脚本, 软件, 以及自己的配置哲学, 如果你对某一部分的配置感兴趣, 欢迎学习他的配置思路, 但是不建议直接copy, 这个仓库本身**不是为了让任何人都能够开箱即用**而发布的
...
- `~/.config/systemd/user/*.service|*.target` 软链到 `~/.lian/systemd/user/`（让 user service 的源文件也留在仓库里）
...
我的 Hyprland 会话里有一些需要长期运行的组件：`waybar`、`swaync`、`fcitx5`、`lianwall`、`cliphist`、`hysp` 等。

早期这些东西都直接写在 `hyprland.conf` 的 `exec-once` 里，配置直观，但排查启动失败、重登后的残留进程、异常退出后的恢复都不太方便。
...
- `~/.config/systemd/user/*.service|*.target` 只作为软链，源文件仍然维护在 `~/.lian`

当前会话入口在 [hypr/hyprland.conf](hypr/hyprland.conf)：
...
systemctl --user status waybar-lbar swaync fcitx5 lianwall
...
如果你按本文推荐的方式维护配置，建议直接把 `~/.config/waybar` 软链到仓库：

```bash
ln -sf ~/.lian/waybar ~/.config/waybar
```

我现在通过 `systemd --user` 启动 Waybar：
