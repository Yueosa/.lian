...
我的环境里有大量自己编写的脚本、自己写的小软件 (`lian` / `lianwall` / `hysp` / `stalk_hypr` / `nmrs` ...)，以及自己一套配置哲学。如果你对其中某一部分的配置感兴趣，**欢迎学习他的配置思路**，但不建议直接 copy。
...
- 🖥️ **GUI 前台服务**（fcitx5 / swaync / waybar / tuxedo-tray / kanshi / polkit-agent）
  → 由 `exec-once` 直接拉起，确保 Wayland session 环境变量和 Socket 完整继承
- ⚙️ **后台常驻服务**（cliphist-watch / hysp / stalk-hypr / lianwall / hypr-event-daemon）
...
如果你抄完本仓库后发现 GTK 应用（`gnome-calendar` / `nautilus` / `pavucontrol` 等）的窗口配色、字体、光标风格和我的截图不一致，通常就是你本机缺少 `~/.config/gtk-3.0/` 与 `~/.config/gtk-4.0/` 的覆盖配置。
...
> 我把图片默认指给了 `imv`、目录指给了 `nautilus`、文本指给了 `nvim` 之类。和你装的软件不一样的话需要改。
...
| | |
|---|---|
| **左侧岛屿** <br> 工作区 / logo / 窗口 <br><img src="./image/waybarleft1.png" alt="waybarleft" width="380" /> | **焦点窗口信息** <br><br><img src="./image/waybarwindow.png" alt="window" width="380" /> |
| **时间显示** <br><br><img src="./image/waybarclock.jpg" alt="clock" width="380" /> | **显卡监控** <br><br><img src="./image/waybargpu.png" alt="gpu" width="380" /> |
| **CPU 监控** <br><br><img src="./image/waybarcpu.png" alt="cpu" width="380" /> | **内存监控** <br><br><img src="./image/waybarmem.png" alt="mem" width="380" /> |
| **媒体信息 / 音频可视化** <br><br><img src="./image/waybarmedia.png" alt="media" width="380" /> | **右侧岛屿** <br> 网络 / 音量 / 电池 / 蓝牙 ... <br><img src="./image/waybarright1.png" alt="waybarright" width="380" /> |
| **网络信息** <br><br><img src="./image/waybarnet.png" alt="net" width="380" /> | **蓝牙信息** <br><br><img src="./image/waybarblt.png" alt="bluetooth" width="380" /> |
| **包管理器** <br><br><img src="./image/waybarup.png" alt="update" width="380" /> | **系统托盘** <br><br><img src="./image/waybartray.png" alt="tray" width="380" /> |
| **托盘右键菜单样式** <br><br><img src="./image/waybaronright.png" alt="onright" width="380" /> | |
...
- 左键：打开 `gnome-calendar`（Hyprland 里我配了浮动窗口规则 `org.gnome.Calendar`）
